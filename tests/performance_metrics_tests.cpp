...
int main() {
  ...
  cost.add(5); cost.refresh(); check(cost.current_ms == 5 && cost.worst_ms == 18);
  cost.reset_average(); check(cost.worst_ms == 18); // solver/reset keeps peak
  cost = {}; check(cost.worst_ms == 0); // only a new application session clears it
}
